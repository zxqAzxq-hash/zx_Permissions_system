...
function loadTree(){
	var url = "/priv/privManage_loadTree";
	var para = {
		ts: new Date().getTime()
	};
		...
		function genPrivTree(menuList, parent){
			for (var i = 0; i < menuList.length; i++) {
				...
				if (menu.type == MENUTYPE_PARENT) {
					...
					$.ajax({
						url: "/priv/privManage_loadAction",
						data: {
							menuId: menu.menuId
						},
						async: false,
...
function matchPriv() {
	...
	//获取当前角色菜单权限与动作权限
	var url = "/priv/privManage_loadPrivs";
	var para = {"roleId": $("#role").val(), "ts":new Date().getTime()};
	...
	$.getJSON(url, para, function(data) {
		...
		for (var i = 0; i < menus.length; i++) {
			$("input:checkbox[rid=" + menus[i] + "]").attr("checked", true);
		}

		for (i = 0; i < actions.length; i++) {
			$("input:checkbox[rid=" + actions[i] + "]").attr("checked", true);
		}
	});
}
...
function savePriv() {
	...
	var url = "/priv/privManage_savePrivs";
